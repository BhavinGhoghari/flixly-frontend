...
export default function SeriesPage() {
  ...
  useEffect(() => {
    tmdb.getTVGenres().then((r) => setGenres(r.data));
  }, []);
  useEffect(() => {
    if (!search) {
      setSearchResults([]);
  ...
  const tabs = [
    ...
    },
    {
      ...
      label: (
        <>
          <ClockCircleOutlined /> On The Air
        </>
      ),
    },
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0f0f0f", paddingTop: 80 }}>
      <div
        ...
        style={{
          background: "linear-gradient(135deg,#0a0a1a 0%,#0f0f0f 100%)",
          padding: "40px 40px 0",
        }}
      >
        <h1
      ...
      >
        ...
        {!search && (
          <div style={{ color: "#555", fontSize: "0.8rem", marginBottom: 16 }}>
            {totalResults.toLocaleString()} series found
          </div>
        )}
        ...
        {loading || searching ? (
          ...
          <Empty
            description={<span style={{ color: "#555" }}>No series found</span>}
            ...
            <div className="movies-grid">
              {displayList.map((m) => (
                <TMDBMovieCard
                  key={m.tmdbId}
                  movie={m}
                  onClick={() => navigate(`/series/${m.tmdbId}`)}
                />
              ))}
            </div>
            {!search && totalPages > 1 && (
