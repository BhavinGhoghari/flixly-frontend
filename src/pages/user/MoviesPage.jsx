...
import { Input, Select, Tabs, Spin, Pagination, Empty, Tag } from 'antd';
import { SearchOutlined, FireFilled, StarFilled, ClockCircleOutlined, CalendarOutlined } from '@ant-design/icons';
...
export default function MoviesPage() {
  ...
  useEffect(() => { tmdb.getMovieGenres().then(r => setGenres(r.data)); }, []);
  useEffect(() => { if (!search) { setSearchResults([]); fetchMovies(); } }, [activeTab, page, selectedGenre, sortBy]);
  useEffect(() => {
    if (!search) return;
  ...
  const fetchMovies = async () => {
    ...
    try {
      ...
      else if (activeTab === 'top_rated') res = await tmdb.getTopRatedMovies(page);
      else if (activeTab === 'upcoming') res = await tmdb.getUpcomingMovies(page);
      else if (activeTab === 'now_playing') res = await tmdb.getNowPlaying(page);
      else if (activeTab === 'discover') {
        res = await tmdb.discoverMovies({ page, sort_by: sortBy, with_genres: selectedGenre });
      }
      const d = res.data;
      setMovies(d.results || []);
  ...
  const tabs = [
    ...
    { key:'now_playing', label:<><ClockCircleOutlined /> Now Playing</> },
    { key:'upcoming', label:<><CalendarOutlined /> Upcoming</> },
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#0f0f0f', paddingTop:80 }}>
      {/* Header */}
      <div className="page-header" style={{ background:'linear-gradient(135deg,#1a0a0a 0%,#0f0f0f 100%)', padding:'40px 40px 0' }}>
      ...
      <div className="page-container" style={{ padding:'20px 40px', maxWidth:1400, margin:'0 auto' }}>
        ...
        <div style={{ display:'flex', gap:12, flexWrap:'wrap', marginBottom:20 }}>
          <Input className="search-input-responsive" prefix={<SearchOutlined style={{ color:'#555' }} />} placeholder="Search movies..."
            value={search} onChange={e => setSearch(e.target.value)} allowClear
            style={{ width:260, background:'#1a1a1a', border:'1px solid #333' }} size="large" />

          {activeTab === 'discover' && <>
            <Select placeholder="Genre" value={selectedGenre || undefined} onChange={v => { setSelectedGenre(v || ''); setPage(1); }}
              allowClear style={{ width:160 }} size="large">
        ...
        {(loading || searching) ? (
          ...
          <Empty description={<span style={{ color:'#555' }}>No movies found</span>} style={{ padding:'60px 0' }} />
        ) : (
          <>
